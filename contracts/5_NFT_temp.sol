...
contract NFT_ERC1155_Temp is ERC1155, Ownable {
    ...
    function mintToken(string memory tokenURI) public onlyOwner returns (uint256) {
        ...
        require(bytes(_uriIdMap[newItemId]).length == 0, "mapping already exist");

        _mint(msg.sender, newItemId, 1, "");  //_mint(account, id, amount, data), data is usually set to ""
